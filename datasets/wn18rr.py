...
class WN18RR(TAGDataset):
    graph_description = "This is a knowledge graph from WordNet. Nodes represent an English word and edges represent the relationship between two words. "
    ...
    def raw_file_names(self) -> list:
        return ["entity2text.txt", "train.txt", "valid.txt", "test.txt", "wn18rr.json"]

    def download(self):
        download_hf_file(HF_REPO_ID, subfolder="WN18RR", filename="entity2text.txt", local_dir=self.raw_dir)
        download_hf_file(HF_REPO_ID, subfolder="WN18RR", filename="test.txt", local_dir=self.raw_dir)
        download_hf_file(HF_REPO_ID, subfolder="WN18RR", filename="valid.txt", local_dir=self.raw_dir)
        download_hf_file(HF_REPO_ID, subfolder="WN18RR", filename="train.txt", local_dir=self.raw_dir)
        download_hf_file(HF_REPO_ID, subfolder="WN18RR", filename="wn18rr.json", local_dir=self.raw_dir)

    def gen_data(self) -> tuple[list[TAGData], Any]:
        entity_lst = []
        text_lst = []
        with open(osp.join(self.raw_dir, "entity2text.txt"), "r") as f:
            lines = f.readlines()
            for line in lines:
                ...
                text_lst.append("English word and its description. Word name: " +
        ...
        for file_type, file_path in name_dict.items():
            ...
            for triplet in file_data:
                ...
                if triplet[1] not in relation2id:
                    ...
                    rel_list.append("Relation from source word to target word: " + triplet[1])
                    rel += 1

                edges.append([entity2id[triplet[2]], entity2id[triplet[0]], ])
        ...
        rel_list = ["Relation from source word to target word: " + label for label in labels]

        data = TAGData(x=text_lst, node_map=node_map, x_original=torch.zeros([len(text_lst), 1]), edge_attr=rel_list,
    ...
    def get_LQA_list(self, label_map: list, **kwargs) -> tuple[list[list], np.ndarray, np.ndarray]:
        ...
        """
        q_list = ["What is the relationship between two target words?"]
